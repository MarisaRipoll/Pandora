...
def train(train_loader, val_loader, model_type='longformer', optim=0, writer=0, lr=5e-5,
          model=0, device=0, tokenizer=0, num_epochs=10, n=5, summary_path=0,
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            optim.zero_grad()
            loss, f1_score, em_score = get_scores(batch, em_score_epoch_train, f1_score_epoch_train, device,
                                                  tokenizer, model, local_only=local_only, global_only=global_only)
        ...
        for i, batch in enumerate(val_loader):
            ...
            loss, f1_score, em_score = get_scores(batch, em_score_epoch_val, f1_score_epoch_val, device,
                                                  tokenizer, model, local_only=local_only, global_only=global_only)
...
def get_scores(batch, em_score_epoch, f1_score_epoch, device, tokenizer, model, local_only=False, global_only=False):
    ...
    if global_only == True:
        ...
        global_attention_mask = torch.zeros(input_ids.shape, dtype=torch.long, device=input_ids.device)
        outputs = model(input_ids, attention_mask=attention_mask, global_attention_mask=global_attention_mask, output_attentions=True, start_positions=start_positions, end_positions=end_positions)
    else: outputs = model(input_ids, attention_mask=attention_mask, output_attentions=True, start_positions=start_positions, end_positions=end_positions)
