...
def train(train_loader, val_loader, model_type='longformer', optim=0, writer=0, lr=5e-5, model=0, device=0, tokenizer=0, num_epochs=10, n=5, summary_path=0, global_att=True):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss, f1_score, em_score = get_scores(batch, em_score_epoch_train, f1_score_epoch_train,
                                                  device, tokenizer, model, global_att=global_att)
    ...
    summary_path=f'{summary_path}_{float(f1_score_train):.3}f1'
...
def get_scores(batch, em_score_epoch, f1_score_epoch, device, tokenizer, model, global_att=True):
    ...
    if global_att==True:
        ...
        global_attention_mask = torch.zeros(input_ids.shape, dtype=torch.long, device=input_ids.device)
        outputs = model(input_ids, attention_mask=attention_mask, global_attention_mask=global_attention_mask, start_positions=start_positions, end_positions=end_positions)
